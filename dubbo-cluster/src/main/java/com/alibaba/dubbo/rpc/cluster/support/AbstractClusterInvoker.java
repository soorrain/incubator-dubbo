...
public abstract class AbstractClusterInvoker<T> implements Invoker<T> {
    ...
    protected Invoker<T> select(LoadBalance loadbalance, Invocation invocation, List<Invoker<T>> invokers, List<Invoker<T>> selected) throws RpcException {
        ...
        boolean sticky = invokers.get(0).getUrl().getMethodParameter(methodName, Constants.CLUSTER_STICKY_KEY, Constants.DEFAULT_CLUSTER_STICKY);
        {
            ...
            // 说明 stickyInvoker 代表的服务提供者挂了，此时需要将 stickyInvoker 置空
            if (stickyInvoker != null && !invokers.contains(stickyInvoker)) {
                stickyInvoker = null;
            }
            //ignore concurrency problem
            // 在 sticky 为 true，且 stickyInvoker != null 的情况。如果 selected 包含 stickyInvoker，
            // 表明 stickyInvoker 对应的服务提供者可能因为网络原因未能成功提供服务，
            // 但是该提供者并没有挂，此时 invokers 列表中仍然存在该提供者对应的 Invoker
        ...
        // 如果线程走到了当前代码出，说明前面的 stickyInvoker 为空，或者不可用。
    ...
    private Invoker<T> doSelect(LoadBalance loadbalance, Invocation invocation, List<Invoker<T>> invokers, List<Invoker<T>> selected) throws RpcException {
        ...
        // 如果 selected 中包含负载均衡组件选择出的 Invoker，或者该 Invoker 无法经过可用性检查，此时进行重选
        ...
        if (availablecheck) { // invoker.isAvailable() should be checked
            ...
            for (Invoker<T> invoker : invokers) {
                ...
                if (invoker.isAvailable()) {
                    // 如果 selected 列表不包含当前 invoker，则将其添加到 reselectInvokers
                    if (selected == null || !selected.contains(invoker)) {
                        reselectInvokers.add(invoker);
                    }
                }
            }
            // reselectInvokers 不为空，此时通过负载均衡组建进行选择
    ...
    protected List<Invoker<T>> list(Invocation invocation) throws RpcException {
        // 调用 directory 的 list 方法，列举 Invoker
